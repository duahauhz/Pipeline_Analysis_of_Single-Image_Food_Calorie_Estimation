...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLOv13n on ECUSTFD locally.")
    parser.add_argument("--data", type=Path, default=Path("datasets/ECUSTFD/ecustfd.yaml"))
    parser.add_argument("--weights", type=Path, default=Path("weights/yolov13n_pretrained.pt"))
    parser.add_argument("--repo", type=Path, default=Path("yolov13"))
    parser.add_argument("--output-root", type=Path, default=Path("runs/local_food_detect"))
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--batch", type=int, default=4)
    parser.add_argument("--imgsz", type=int, default=640)
    parser.add_argument("--workers", type=int, default=2)
    parser.add_argument("--patience", type=int, default=50)
    parser.add_argument("--name", default="yolov13n_ecustfd_local")
...
def main() -> None:
    ...
    data_yaml = write_local_yaml(data_yaml, args.output_root)
    ...
    output_dir = args.output_root / "output"
    output_dir.mkdir(parents=True, exist_ok=True)
